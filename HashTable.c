...
// defines our helper function so we can use it in HashTable_Insert
static bool Iterate_BucketKeys(LLIterator *iter,
  HTKeyValue_t **keyvalue, HTKey_t findkey);
...
bool HashTable_Insert(HashTable *table,
  ...
  if (LinkedList_NumElements(chain) == 0) {
    LinkedList_Append(chain, (void*) newpayload);
    table -> num_elements++;
    return false;
  }
  ...
  if (!Iterate_BucketKeys(iter, &oldpayload, newkeyvalue.key)) {
    LinkedList_Append(chain, (void*) newpayload);
    table -> num_elements++;
    free(iter);
    return false;
...
// Helper function
// Iterate through a bucket's keys and return true if we find 
// a specific key in the list, else return false
static bool Iterate_BucketKeys(LLIterator *iter,
  HTKeyValue_t **keyvalue, HTKey_t findkey) {
  ...
  // newkey is not present
  // we've reached past the end of the list
  return false;
}
...
bool HashTable_Find(HashTable *table,
  ...
  LLIterator *iter = LLIterator_Allocate(chain);
  HTKeyValue_t* temp;
  if (!Iterate_BucketKeys(iter, &temp, key)) {
    free(iter);
    return false;
  }
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  HTKeyValue_t* temp;
  LLIterator_Get(iter -> bucket_it, (void**)&temp);
  keyvalue -> key = temp -> key;
  keyvalue -> value = temp -> value;
