...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  // if key can't be found, free the linkedlist
  ...
  while (HTIterator_Get(it, &kv)) {
    SearchResult* match = (SearchResult*) malloc(sizeof(SearchResult));
    match->doc_id = kv.key;
    match->rank = LinkedList_NumElements(kv.value);
    LinkedList_Append(ret_list, match);
    HTIterator_Next(it);
  }
  HTIterator_Free(it);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      // if we find the match, increment by number of occurances (rank)
      if (HashTable_Find(wp->postings, match->doc_id, &kv)) {
        match->rank += LinkedList_NumElements(kv.value);
